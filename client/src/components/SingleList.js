...
export class SingleList extends Component {
  ...
  statusRender = (status) => {
    switch (status) {
      case "Done":
        status = "Done";
        return <span className="text-left badge badge-success">{status}</span>;
      case "InProgress":
        status = "In Progress";
        return <span className="text-left badge badge-warning">{status}</span>;
      case "NotStarted":
        status = "Not Started";
            ...
            <CardBody>
              ...
              <CardSubtitle>
                <h5 className="text-muted">
                  Select a todo or create a new todo to get started{" "}
                </h5>
              </CardSubtitle>
              <br />
              <ListGroup>
                {currList.todos.map((todo) => (
                  <ListGroupItem
                    className=" my-3  todoStyle p-0"
                    key={todo._id}
                    onClick={this.todoModal.bind(this, currList._id, todo._id)}>
                    <CardHeader>
                    ...
                    <CardBody className="p-3">
                      ...
                      {!todo.desc ? (
                        <p className="text-left">No Description Given.</p>
                      ) : (
                        <p className="text-left w-75 text-muted">{todo.desc}</p>
                      )}
                      <br />
                    </CardBody>
              ...
              {Object.keys(this.props.currTodo).length !== 0 ? (
                <TodoModal
                  modal={this.state.isOpen}
                  toggle={this.toggle}
                  list={this.props.currList}
                  todo={this.props.currTodo}
                  checklist={this.props.currTodo.checklist}></TodoModal>
              ) : null}
              ...
              <Form onSubmit={this.onSubmit}>
                <FormGroup>
                  ...
                  <Button className="w-75" size="lg" color="primary">
                    {" "}
                    <FontAwesomeIcon icon={faPlus} /> &nbsp; Add a New ToDo{" "}
                  </Button>
                </FormGroup>
              </Form>
            </CardBody>
...
export default connect(
  mapStateToProps,
  { getSingleList, createNewTodo, deleteOneTodo, getOneTodo }
)(SingleList);
